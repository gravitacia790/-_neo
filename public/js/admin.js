function renderAdminPanel() {
  ...
  if (!container) return;
  if (!isAdmin()) { container.innerHTML = '<div>Доступ запрещён</div>'; return; }
  container.innerHTML = '<h2>👑 Админ-панель</h2><div>Загрузка...</div>';
  API.getAdminUsers().then(function (resp) {
    var users = resp.users;
    var html = '<h2>👑 Админ-панель: рейтинг директоров</h2>' +
    ...
    users.forEach(function (u) {
      var lastActs = (u.lastActivities || []).map(function (a) {
      ...
      html += '<tr><td>' + escapeHtml(u.name) + '</td><td>' + escapeHtml(u.email) + '</td>' +
        ...
        '<td>' + (u.public ? 'Да' : 'Нет') + '</td>' +
        '<td><small>' + lastActs + '</small></td></tr>';
    });
