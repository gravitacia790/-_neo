...
import { addSkill, addStrength, clearFormErrors, initPhotoUpload, setPendingPhotoFile } from './profile-ui.js';
import { notify, setMyProfileCache } from './utils.js';
import { html, setHtml } from './html.js';
export function loadUserDataIntoForms() {
  ...
  return retryPromise(function () {
    ...
    .then(function (resp) {
      ...
      if (photoPreview) {
        setHtml(photoPreview, profile.photo ? html`<img src="${profile.photo}" alt="Фото профиля">` : html``);
      }
      ...
      if (sv && school.name) {
        ...
        setHtml(
          sv,
          html`<div class="school-card"><div class="school-field"><strong>Название:</strong> ${school.name}</div><div class="school-field"><strong>Адрес:</strong> ${school.address || '—'}</div><div class="school-field"><strong>Учеников:</strong> ${school.students || '—'}</div><div class="school-field"><strong>Педагогов:</strong> ${school.teachers || '—'}</div><div class="school-field"><strong>Тип:</strong> ${school.type || '—'}</div><div class="school-field"><strong>Зданий:</strong> ${school.buildingCount || '—'}</div><button class="save-btn" id="editSchoolBtn" style="margin-top:12px;">Редактировать</button></div>`
        );
      ...
      initPhotoUpload();
      if (typeof MAX_LINK !== 'undefined' && MAX_LINK) MAX_LINK.refresh();
    })
    .catch(function (err) {
