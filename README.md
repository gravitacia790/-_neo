...
Это закрытый клуб с доступом по приглашениям. Ориентир в 1500 — верхняя граница зарегистрированных аккаунтов, а не количество одновременных пользователей. Единые термины и границы текущей реализации зафиксированы в [`docs/PROJECT_CONTEXT.md`](docs/PROJECT_CONTEXT.md).
...
- при включённом `DEMO_MODE=true` или `SEED_DEMO=true` создаются 4 демо-директора (`elena@school11.ru` / `demo1234`)
...
### Render (временный тестовый стенд)
...
3. Render создаст сервис приложения и базу PostgreSQL.
4. В переменных окружения задайте:
   - `ADMIN_EMAIL`
   - `ADMIN_PASSWORD` (сильный, 10+ символов)
5. Дождитесь первого деплоя и откройте URL сервиса.

Render используется для тестирования текущей версии и проверки сценариев перед переносом на постоянную площадку. Это не целевая production-инфраструктура.

Важно:
- `DATABASE_URL` подключается к PostgreSQL из Blueprint;
- `JWT_SECRET` генерируется автоматически Blueprint-ом;
- при free-plan сервис может "засыпать", первый запрос после сна медленнее; такой режим подходит только для тестирования.
...
Браузерные тесты через `npm run test:e2e` сами запускают отдельный сервер на порту `3100` с временной базой `pg-mem`. Они не используют development или production PostgreSQL.
...
Чек-лист постоянного запуска закрытого клуба до 1500 зарегистрированных аккаунтов: [`docs/GO_LIVE_1500.md`](docs/GO_LIVE_1500.md).
Продуктовый контекст и спорные термины: [`docs/PROJECT_CONTEXT.md`](docs/PROJECT_CONTEXT.md).
